...
export function SeriesPage() {
  ...
  if (state.status === 'error') return <main className={mainCls}>
  ...
  return (
    <main className={mainCls}>
      ...
      >
        {coverSrc && (
          <div
            className="aspect-[16/9] rounded-md overflow-hidden bg-bg-soft border border-rule max-sm:max-w-full">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3 text-sm text-fg-3">
            <span
              className="inline-block py-[3px] px-[10px] bg-accent-soft text-accent rounded-pill text-xs font-semibold tracking-[0.04em]">{manifest.status}</span>
      ...
      <div className="flex gap-2 border-b border-rule mb-6 relative flex-wrap" role="tablist"
        ...
        <button role="tab" id="tab-characters" aria-controls="panel-characters"
                aria-selected={tab === 'characters'} className={tabBtnCls}
        ...
        {isAuthor && (
          <button role="tab" id="tab-author" aria-controls="panel-author"
                  aria-selected={tab === 'author'} className={tabBtnCls}
      ...
      {tab === 'overview' && (
        <article role="tabpanel" id="panel-overview" aria-labelledby="tab-overview"
                 className="article article-wiki" dangerouslySetInnerHTML={{__html: bodyHtml}}/>
      )}
      {tab === 'chapters' && (
