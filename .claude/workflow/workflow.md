...
다음 조건 중 **하나 이상** 충족 시 본 워크플로우 적용 강제 *(2026-05-19 범위 확장)*:

1. 사용자가 `auto-mode` / `자율 진행` / `중단 요청 전까지 계속` 등 명시
2. 사용자가 본 디렉토리 (`.claude/workflow/`) 의 template 을 직접 참조
3. **모든 챕터 / 캐릭터 카드 / 세계관 문서 작성·개선** *(글 작성도 예외 없음 — Context 단계에서 `writing-principles.md` + 등장 카드
   Read 필수)*
4. **모든 신규 기능 / 컴포넌트 / 페이지 / 슬라이스 도입**
5. **모든 개선 / 리팩토링 / 튜닝 / 버그 정정**
6. **모든 SSOT 갱신** (`CLAUDE.md` / `harness-state.md` / `_series.md` / 신규 manifest 필드 등)

**면제** = 단일 파일 1줄 정정 / 검색 / 읽기 / 단순 질문 응답.

본 흐름을 우회하면 안 됨. 위반 시 사용자가 "workflow 따라" 명시로 재진입 요청.
