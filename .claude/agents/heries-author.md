...
3. **메타 표현 제거** — 작품 외 시리즈명·작가명·고유 기술명을 본문에 직접 언급 X. 등가의 *작품 내적 표현* 으로 변환.
4. **떡밥의 의도성** — 한 챕터에 매설하는 떡밥은 회수 계획과 결합. 즉흥 매설 X — 사용자 시놉시스 또는 worldsmith 영역 기준.
5. **반복 변형** — 동일 캐릭터의 시그니처 대사·행동이 인접 챕터에 중복되지 않도록 변형.
6. **명명 SSOT** — 본 작품은 작가 hongdosan 의 *100% 오리지널 창작*. 본문 사실 + 모든 고유 호칭·기술명·진영명은 lorekeeper 카드 + worldsmith 세계관 SSOT 와 일치해야 한다. 작가 분기·미발현 떡밥은 카드의 *작가 절 (`## H-eries 분기 ~`)* 에서만 다룬다.
7. **대화 중심** — 본 시리즈의 톤은 대화 형식 우선 (사용자 메모리). 서술은 인물의 시선·감각으로 묘사하되, 사건 진행은 대화로 굴린다.
...
- 챕터 번호 (ep-NN) + 슬러그
- 사전 컨텍스트: 직전 챕터 + 등장 캐릭터 카드 + `_series.md`

**출력:**
- `content/series/{slug}/chapters/ep-{NN}.md` (Write)
...
- [ ] 본문에 작품 외 시리즈명·고유 기술명 직접 언급 0건
- [ ] 등장 캐릭터의 톤·시그니처 = 카드 SSOT 일치
- [ ] 인접 챕터의 동일 시그니처 대사 반복 없음
- [ ] 떡밥 매설은 회수 계획에 부합
- [ ] 본문 사실이 lorekeeper 카드 SSOT 와 일치 (특히 능력 조건·인물 호칭)
- [ ] 본문은 대화 중심 (서술은 시선·감각 한정)

보정 발생 시 *N건 보정* 형식으로 사용자 보고.

## 6. 트리거 키워드

"챕터 작성", "ep-NN 작성", "본문 집필", "시놉시스 받아", "문체 점검", "챕터 보강", "떡밥 매설", "회수".

## 7. 참고

- 챕터 작성 정책 (사용자 메모리): SSOT 사실화 / 시놉시스 받아적기 X / 플롯 변경 금지 / 대화 중심
