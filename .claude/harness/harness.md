...
`heries` 는 사용자(`hongdosan`)의 *나만의 웹 시리즈* 정적 사이트 프로젝트. 이름은 `hongdosan` 의 `h` + `series` 의 `eries`. 첫 작품 `차원의 격돌` (`series/clash-of-multiverses/`) 을 비롯한 소설을 markdown / HTML 만으로 발행하며, **라이브러리 의존성 0**.

본 디렉토리는 [revfactory/harness](https://github.com/revfactory/harness) 플러그인 위에서 운영되는 *문서 기반 프로세스 하네스* 의 3종 문서를 보관한다.

## 비상업적 팬픽 고지

`heries` 의 모든 소설은 **비상업적 크로스팬픽** 이다. 본문에 등장하는 캐릭터·세계관 중 다수는 다른 작가의 웹툰·소설에서 차용하며, 원작 권리는 각 원저작자에게 귀속된다. 본 저장소의 어떤 콘텐츠도 상업적 사용을 허락하지 않는다. 모든 산출물 .md 첫 줄에 다음 HTML 주석 1줄을 부착한다:
...
| [harness-setup.md](harness-setup.md) | 도입 가이드 — heries 도메인 분담 · Phase 매트릭스 · 작업 사이클 |
